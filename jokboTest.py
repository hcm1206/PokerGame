...
for i in range(10): # 랜덤으로 7장 선택한 5가지 패 (UI로 시각화)
    ...
    resultText = "스트레이트☆★☆\n" if straight(select) else "스트레이트 아님\n"
    resultText += "플러시☆★☆" if flush(select) else "플러시 아님"
    Label(window, text=resultText).grid(row=i,column=7)
    # 여기까지 GUI용 ========================================================
